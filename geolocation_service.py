...
RODOVIAS_DB = {
    ...
    "BR-060 (Brasília-Mato Grosso do Sul)": [(-16.68, -49.25), (-17.79, -50.93)],
    "BR-364 (Sudeste a Noroeste)": [(-17.88, -51.71), (-16.33, -54.69)],
...
def get_soil_data(lat, lon):
    ...
    try:
        ...
        if ph_value is not None and clay_value is not None:
            ...
            st.warning(f"Não foi possível obter dados de solo da API. Usando valores padrão.")
            return True, DEFAULT_SOIL_DATA
    except Exception:
        st.warning(f"Não foi possível buscar dados de solo. Usando valores padrão.")
        return True, DEFAULT_SOIL_DATA
...
def find_local_pois(lat, lon, return_coords=False):
    ...
    query = f"""[out:json][timeout:45];((node["place"~"city|town|village"](around:{raio_local_m},{lat},{lon}););(node["man_made"="silo"](around:{raio_local_m},{lat},{lon});node["name"~"Silo|Cooperativa|Graneleiro",i](around:{raio_local_m},{lat},{lon});););out center;"""
    for endpoint in OVERPASS_ENDPOINTS:
        try:
            response = requests.post(endpoint, data=query, timeout=60)
            response.raise_for_status()
            data = response.json()
            ...
            for element in data['elements']:
                tags, name = element.get('tags', {}), element.get('tags', {}).get('name', 'Ponto de Interesse')
                ...
                elif ("man_made" in tags or any(k in name for k in ["Silo", "Cooperativa", "Graneleiro"])) and dist < min_dists["silo"]:
                    min_dists["silo"] = dist
