...
st.set_page_config(page_title="AgroScore Validator 4.1", page_icon="🛰️", layout="wide")

# --- Título e Descrição ---
st.title("🛰️ AgroScore Validator 4.1")
...
with st.sidebar:
    ...
    st.info("As distâncias podem ser preenchidas automaticamente ao clicar em 'Analisar', ou ajustadas manualmente.")
...
if analisar:
    all_pois = None
    hub = None
    with st.spinner("Buscando dados geográficos e logísticos... (Pode levar até 1 minuto)"):
        all_pois = find_all_nearest_pois(latitude, longitude, return_coords=True)
        hub = find_nearest_hub(latitude, longitude)
    
    # MUDANÇA AQUI: Lógica mais robusta
    # Se a busca automática funcionou, usa os dados dela.
    if all_pois:
    ...
    # Se a busca falhou, usa os dados que já estavam nos campos manuais.
    else:
        dist_rodovia_final = st.session_state.dist_rodovia
        dist_silo_final = st.session_state.dist_silo
        # A mensagem de erro já é exibida pelo geolocation_service
    ...
    with tab1:
        # ... (O conteúdo desta aba continua o mesmo)
    ...
    with tab2:
        # ... (O conteúdo do mapa e das distâncias continua o mesmo)
